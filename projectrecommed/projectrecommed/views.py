...
def reco(request):
    if request.method == 'GET':
        ...
        category = user_data_order_dict.get('category', 'default_category')
        if category is None:
            raise ValueError('Category is required in the request parameters.')


        user_dict = {
            k: 0 if not v[0] else quantify_age(int(v[0])) if k == 'age' else int(v[0])
            for k, v in user_data_order_dict.items() if v[0]
        }
